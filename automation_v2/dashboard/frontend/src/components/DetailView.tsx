...
function fmtImp(val: number | string | undefined) {
  if (!val) return 'TBD';
  
  // If it's a string with units (e.g. "3.2 Mn"), return as is
  if (typeof val === 'string' && /[a-zA-Z]/.test(val)) return val;
  
  const n = typeof val === 'string' ? parseFloat(val.replace(/,/g, '')) : val;
  if (isNaN(n) || n === 0) return 'TBD';
...
export function DetailView({ detail, loading, onBack }: DetailViewProps) {
  ...
  // Calculate sum only for numeric-only metrics, otherwise take the first surgical one
  const totalImpVal = detail.Metrics?.reduce((s, m) => {
    const raw = (m.Impressions || m.TAM || '0');
    // If it has Mn/Cr, we handle it as a single source of truth (don't sum with others)
    if (/[a-zA-Z]/.test(raw)) return s; 
    const n = parseFloat(raw.replace(/[^0-9.]/g, ''));
    return s + (isNaN(n) ? 0 : n);
  }, 0) || 0;

  const displayImp = (primaryMetric?.Impressions && /[a-zA-Z]/.test(primaryMetric.Impressions))
    ...
    <div className="detail-view">
      ...
      <div className="detail-kpis">
        <div className="detail-kpi-card">
          ...
          <div>
            <div className="detail-kpi-val">{displayImp}</div>
        ...
        <div className="detail-kpi-card">
          ...
          <div>
            <div className="detail-kpi-val">{primaryMetric?.LTV_Spots || detail.LTV_Spots || '—'}</div>
        ...
        <div className="detail-kpi-card">
          ...
          <div>
            <div className="detail-kpi-val" style={{ fontSize: '0.9rem' }}>
              {formatDate(primaryMetric?.Start_Date || detail.Actual_Live_Date || detail.Campaign_Live_Date)} → {formatDate(primaryMetric?.End_Date || detail.Actual_End_Date || detail.Campaign_End_Date)}
            </div>
